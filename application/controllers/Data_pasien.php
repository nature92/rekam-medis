...
class Data_pasien extends CI_Controller {
	...
	public function hapus($kd_rm){
		...
		if($kodeperiksa == '' or $kodeperiksa == null){
			...
			$this->session->set_flashdata('msg', '<div class="alert alert-error"> 
												  <button type="button" class="close" data-dismiss="alert">&times;</button>Gagal hapus, data pemeriksaan belum dihapus.
												</div>');
			redirect('data_pasien');
		}
	}
	...
	public function ubah($kd_rm){
		...
		$data['admin'] = $this->db->get_where('admin',['username' => $this->session->userdata('username')])->row_array();
		$this->form_validation->set_rules('kd_rm', 'KD RM', 'required');
		$this->form_validation->set_rules('nama_pasien', 'Nama Pasien', 'required');
		$this->form_validation->set_rules('tempat_lahir', 'Tempat Lahir', 'required');
		$this->form_validation->set_rules('tanggal_lahir', 'Tanggal Lahir', 'required');
		$this->form_validation->set_rules('alamat', 'Alamat', 'required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->Pasien_model->ubah_data($kd_rm);
			redirect('data_pasien/index');
		}
	}
