...
class Cetak extends CI_Controller {
	...
	public function cetak($kd_rm) {
		$data = array(
					  ...
					  'dokter'  => $this->db->query("SELECT * FROM pemeriksaan inner join dokter on dokter.id_dokter = pemeriksaan.id_dokter where pemeriksaan.kd_rm='$kd_rm'"),
					);
	...
	public function cetak_bayar($kodebayar) {
		...
		$data['periksa'] = $this->db->query("SELECT * FROM pembayaran JOIN pemeriksaan on pemeriksaan.id_periksa = pembayaran.id_pemeriksaan where pembayaran.kd_bayar = '$kodebayar'")->row_array();
